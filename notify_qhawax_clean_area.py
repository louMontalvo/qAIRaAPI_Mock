...
NOTIFY_MAINTENANCE_URL = BASE_URL + 'api/qhawax_notify_qhawax_cleaning_area/'
...
for qhawax_installed in json_data:
	if(qhawax_installed['last_cleaning_area_date']==None):
		...
		type_date = 'Zona'
		qhawax_last_timestamp = dateutil.parser.parse(qhawax_installed['last_cleaning_area_date'])
	if isQhawaReadyToMaintenance(qhawax_last_timestamp+datetime.timedelta(0.5*360/12), datetime.datetime.now()-datetime.timedelta(hours=5)):
