...
NOTIFY_MAINTENANCE_URL = BASE_URL + 'api/qhawax_notify_qhawax_maintenance/'
...
for qhawax_installed in json_data:
	if(qhawax_installed['last_maintenance_date']==None):
		type_date = 'Instalacion'
		qhawax_last_timestamp= dateutil.parser.parse(qhawax_installed['instalation_date'])
	else:
		type_date = 'Mantenimiento'
		qhawax_last_timestamp = dateutil.parser.parse(qhawax_installed['last_maintenance_date'])
	if isQhawaReadyToMaintenance(qhawax_last_timestamp+datetime.timedelta(3*360/12), datetime.datetime.now()-datetime.timedelta(hours=5)):
