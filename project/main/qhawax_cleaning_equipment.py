...
@app.route('/api/qhawax_notify_qhawax_cleaning_equipment/', methods=['POST'])
def sendQhawaxCleaningEquipmentNotifyTimestamp():
    ...
    if installation_id is not None and bcrypt.verify(app.config['SECRET_KEY'], secret_key_hashed):
        subject = 'Limpieza del Equipo en %s' % (comercial_name)
        if(type_date =='Instalacion'):
            date = utils.getInstallationQhawaxDate(db.session,installation_id)
            content = 'Fecha de Instalacion: %s \n Qhawax listo para su primera limpieza del equipo' % (date)
        elif (type_date =='Equipo'):
            date = utils.getCleaningEquipmentQhawaxDate(db.session,installation_id)
